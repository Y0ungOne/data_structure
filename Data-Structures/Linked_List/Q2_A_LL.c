...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	int index = 1;
	while (cur2 != NULL)					// ll2을 넣기 때문에 넣을 ll2가 없다면 종료되야함
	{
		insertNode(ll1, index, cur2->item); // ll2 노드값 ll1에 삽입
		removeNode(ll2, 0);					// 삽입한 ll2노드값은 ll2에서 삭제
		index += 2;							// 삽입할 다음 위치(ll1을 건너뛰어야 하기 때문에 2칸씩 건너뜀)
		cur2 = ll2->head;					// ll2 첫 노드 갱신
	}
}
